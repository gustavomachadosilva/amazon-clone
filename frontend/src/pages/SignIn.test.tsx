...
function renderSignIn(route: string | Partial<Location> = '/signin') {
...
describe('SignIn page', () => {
  ...
  it('returns to the page the user was sent from after signing in', async () => {
    mockSuccessfulLogin()

    renderSignIn({ pathname: '/signin', state: { from: { pathname: '/orders' } } })

    fireEvent.change(screen.getByLabelText('Email'), { target: { value: 'buyer@example.com' } })
    fireEvent.change(screen.getByLabelText('Password'), { target: { value: 'password123' } })
    fireEvent.click(screen.getByText('Continue'))

    await waitFor(() => expect(screen.getByText('Orders stub')).toBeInTheDocument())
  })
})
