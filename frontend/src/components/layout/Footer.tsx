...
export default function Footer() {
  return (
    <footer className="mt-10 bg-accent-900 text-paper-50">
      <div className="grid w-full grid-cols-2 gap-6 px-4 py-8 sm:grid-cols-4 md:px-8 md:py-[38px] lg:px-10">
        {COLUMNS.map((column) => (
          <div key={column.title}>
            <div className="h mb-3 text-[18px] uppercase text-paper-50">{column.title}</div>
            <ul className="m-0 flex list-none flex-col gap-2 p-0">
              {column.items.map((item) => (
                <li key={item.label} className="text-[16.5px] text-accent-200">
                  {item.to ? (
                    <Link to={item.to} className="text-accent-200 hover:text-paper-50 hover:underline">
                      {item.label}
                    </Link>
                  ) : (
                    item.label
      ...
      <div className="border-t border-accent-700 px-4 py-4 text-center">
        <span className="font-mono text-[14.5px] uppercase tracking-wide text-accent-300">
